...
export default function Index() {
  ...
  const search = Object.values(movieQuotes)
    .filter(x => new RegExp(quote, "i").test(x.sub))
    .slice(-10);
        ...
        {quote.length
          ? search.map(subs => (
              <Popup
                ...
                trigger={<ul className="search-results">{subs.sub}</ul>}
                modal
              ...
              >
                <video
                  className="modal-gif"
                  src={currentQuote.gif}
                  alt={currentQuote.sub}
                  preload="true"
                  loop
                  autoPlay
                />
                ...
                >
                  {quoteIndex ? (
                    ...
                    <span className="btn-disabled" disabled>
                      {previousButton}
                    </span>
                  )}
...
const ShareButtons = props => {
  ...
  return (
    ...
    >
      <TumblrShareButton
        url={currentQuote.gif}
        title={`"${currentQuote.sub}" via thanosgenerator.com`}
        windowWidth={660}
        windowHeight={460}
      >
      ...
      <FacebookShareButton
        url={currentQuote.gif}
        quote={`"${currentQuote.sub}" via thanosgenerator.com`}
      >
